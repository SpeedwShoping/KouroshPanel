...
kourosh_update_run_id="${XUI_UPDATE_RUN_ID:-0}"
[[ "${kourosh_update_run_id}" =~ ^[0-9]+$ ]] || kourosh_update_run_id="0"
kourosh_update_status_file="${XUI_UPDATE_STATUS_FILE:-/etc/kourosh/update-status.json}"

_write_update_status() {
    local state="$1"
    local exit_code="$2"
    local status_dir
    status_dir="$(dirname "${kourosh_update_status_file}")"
    mkdir -p "${status_dir}" > /dev/null 2>&1
    local tmp_file="${kourosh_update_status_file}.tmp.$$"
    printf '{"runId":"%s","state":"%s","exitCode":%s,"finishedAt":%s}\n' \
        "${kourosh_update_run_id}" "${state}" "${exit_code}" "$(date +%s)" > "${tmp_file}" 2> /dev/null
    mv -f "${tmp_file}" "${kourosh_update_status_file}" > /dev/null 2>&1
}
...
load_kourosh_env() {
    local env_file
    env_file="$(kourosh_env_file_path)"
    if [[ -r "$env_file" ]]; then
        set -a
...
prompt_and_setup_ssl() {
    ...
    case "$ssl_choice" in
        ...
        2)
            ...
            if [[ $release == "alpine" ]]; then
                rc-service kourosh restart > /dev/null 2>&1
            else
                systemctl restart kourosh > /dev/null 2>&1
            fi
...
config_after_update() {
    ...
    echo -e "${yellow}kourosh settings:${plain}"
    ${kourosh_folder}/kourosh setting -show true
    ${kourosh_folder}/kourosh migrate

    # Properly detect empty cert by checking if cert: line exists and has content after it
    local existing_cert=$(${kourosh_folder}/kourosh setting -getCert true 2> /dev/null | grep 'cert:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
...
# by invoking the freshly downloaded kourosh CLI. IP Limit is load-bearing on
...
# the update. XUI_ENABLE_FAIL2BAN is honored (load_kourosh_env exports it from the
...
update_kourosh() {
    ...
    if [ -f "${kourosh_folder}/kourosh" ]; then
        current_kourosh_version=$(${kourosh_folder}/kourosh -v)
        echo -e "${green}Current kourosh version: ${current_kourosh_version}${plain}"
    else
        _fail "ERROR: Current kourosh version: unknown"
    fi

    echo -e "${green}Downloading new kourosh version...${plain}"
    ...
    if [[ ! -s ${kourosh_folder}-linux-$(arch).tar.gz ]]; then
        ...
        _fail "ERROR: Downloaded kourosh release archive is empty, please be sure that your server can access GitHub"
    fi

    if [[ -e ${kourosh_folder}/ ]]; then
        echo -e "${green}Stopping kourosh...${plain}"
        if [[ $release == "alpine" ]]; then
            if [ -f "/etc/init.d/kourosh" ]; then
                rc-service kourosh stop > /dev/null 2>&1
                rc-update del kourosh > /dev/null 2>&1
            ...
            if [ -f "${kourosh_service}/kourosh.service" ]; then
                systemctl stop kourosh > /dev/null 2>&1
                systemctl disable kourosh > /dev/null 2>&1
        ...
        echo -e "${green}Removing old kourosh version...${plain}"
        rm ${kourosh_folder} -f > /dev/null 2>&1
        rm ${kourosh_folder}/kourosh.service -f > /dev/null 2>&1
        rm ${kourosh_folder}/kourosh.service.debian -f > /dev/null 2>&1
        ...
        rm ${kourosh_folder}/bin/xray-linux-$(arch) -f > /dev/null 2>&1
    ...
    chmod +x kourosh bin/xray-linux-$(arch) > /dev/null 2>&1
    ...
    chown -R root:root ${kourosh_folder} > /dev/null 2>&1

    if [ -f "${kourosh_folder}/bin/config.json" ]; then
        echo -e "${green}Changing on config file permissions...${plain}"
        chmod 640 ${kourosh_folder}/bin/config.json > /dev/null 2>&1
    fi
    ...
    if [[ $release == "alpine" ]]; then
        ...
        if [[ ! -s "${kourosh_rc_temp}" ]]; then
            rm -f "${kourosh_rc_temp}"
            _fail "ERROR: Downloaded startup unit kourosh.rc is empty, please be sure that your server can access GitHub"
        fi
        mv -f "${kourosh_rc_temp}" /etc/init.d/kourosh
        if [[ $? -ne 0 ]]; then
            rm -f "${kourosh_rc_temp}"
            _fail "ERROR: Failed to install startup unit kourosh.rc"
        fi
        chmod +x /etc/init.d/kourosh > /dev/null 2>&1
        chown root:root /etc/init.d/kourosh > /dev/null 2>&1
        rc-update add kourosh > /dev/null 2>&1
        rc-service kourosh start > /dev/null 2>&1
    else
        if [ -f "kourosh.service" ]; then
            echo -e "${green}Installing systemd unit...${plain}"
            if ! _install_kourosh_service_unit "kourosh.service" "false"; then
                echo -e "${red}Failed to copy kourosh.service${plain}"
                exit 1
            fi
        else
        ...
        systemctl daemon-reload > /dev/null 2>&1
        systemctl enable kourosh > /dev/null 2>&1
    ...
    echo -e "${green}kourosh ${tag_version}${plain} updating finished, it is running now..."
    echo -e ""
    echo -e "┌───────────────────────────────────────────────────────┐
