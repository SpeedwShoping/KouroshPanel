# Kourosh Panel on Hetzner Cloud

Hetzner Cloud does **not** have a third-party image marketplace the way AWS does.
Ship Kourosh Panel via **cloud-init**: each instance installs non-interactively and
...
Kourosh Panel non-interactively and generates unique per-instance credentials.
...
```bash
ssh root@<server-ip> 'cat /etc/kourosh/install-result.env'
```
...
is essentially a documented cloud-init config plus metadata. To propose Kourosh Panel as
