...
#   * /etc/kourosh/install-result.env exists (mode 600) with random, non-default creds
...
docker run --rm \
    ...
    "$IMAGE" bash -euo pipefail -c '
        ...
        /usr/local/kourosh/kourosh setting -show | grep -q "hasDefaultCredential: false" \
            || { echo "FAIL: hasDefaultCredential is not false"; exit 1; }

        echo "--- verifying the panel serves HTTP ---"
        cd /usr/local/kourosh
        ./kourosh > /tmp/kourosh.log 2>&1 &
        xpid=$!
        ...
        case "${code:-}" in
            200|301|302|307|308) : ;;
            *) echo "FAIL: panel did not serve (status ${code:-none})"; tail -n 30 /tmp/kourosh.log; exit 1 ;;
        esac
