...
type Manager struct {
	...
	// processes (survivors of a previous kourosh run) has already run.
	swept bool
}
...
// sweepOrphansLocked kills mtg processes left running by a previous kourosh run,
// exactly once per process lifetime and before any of our own mtg are started.
// Because kourosh owns every mtg process, anything alive at this point is an orphan
