...
const (
	panelUpdaterURL      = "https://raw.githubusercontent.com/MHSanaei/Kourosh/main/update.sh"
...
// GetUpdateInfo checks GitHub for the latest Kourosh release. When the dev channel
...
func (s *PanelService) startUpdate(useDev bool) (int64, error) {
	...
	if systemdRun, err := exec.LookPath("systemd-run"); err == nil {
		unitName := fmt.Sprintf("kourosh-web-update-%d", time.Now().Unix())
...
func downloadPanelUpdater() (string, error) {
	...
	file, err := os.CreateTemp("", "Kourosh-update-*.sh")
	if err != nil {
		return "", err
	}
	path := file.Name()
...
func fetchPanelRelease(tag string) (*service.Release, error) {
	url := "https://api.github.com/repos/MHSanaei/Kourosh/releases/latest"
	if tag != "" {
		url = "https://api.github.com/repos/MHSanaei/Kourosh/releases/tags/" + tag
	}
