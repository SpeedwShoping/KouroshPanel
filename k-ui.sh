...
if [[ "${running_in_docker}" == "true" ]]; then
    kourosh_folder="${XUI_MAIN_FOLDER:=/app}"
else
    kourosh_folder="${XUI_MAIN_FOLDER:=/usr/local/kourosh}"
fi
kourosh_service="${XUI_SERVICE:=/etc/systemd/system}"
log_folder="${XUI_LOG_FOLDER:=/var/log/kourosh}"
mkdir -p "${log_folder}"
...
update() {
    confirm "This function will update all kourosh components to the latest version, and the data will not be lost. Do you want to continue?" "y"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/SpeedwiT/KouroshPanel/main/update.sh)
    if [[ $? == 0 ]]; then
...
update_dev() {
    confirm "This will update kourosh to the latest DEV commit (the rolling 'dev-latest' build, not a stable release). Your data is preserved. Continue?" "y"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
    ...
    XUI_UPDATE_TAG="dev-latest" bash <(curl -Ls https://raw.githubusercontent.com/SpeedwiT/KouroshPanel/main/update.sh)
    if [[ $? == 0 ]]; then
...
update_menu() {
    ...
    if replace_kourosh_script "https://raw.githubusercontent.com/SpeedwiT/KouroshPanel/main/k-ui.sh" "false"; then
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/SpeedwiT/KouroshPanel/v$tag_version/install.sh") v$tag_version"
...
kourosh_env_file_path() {
    case "${release}" in
        ubuntu | debian | armbian)
            echo "/etc/default/kourosh"
            ;;
        arch | manjaro | parch | alpine)
            echo "/etc/conf.d/kourosh"
            ;;
        *)
            echo "/etc/sysconfig/kourosh"
            ;;
    esac
}
...
uninstall() {
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rc-update del kourosh
        rm /etc/init.d/kourosh -f
    else
        systemctl stop kourosh
        systemctl disable kourosh
        rm ${kourosh_service}/kourosh.service -f
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/SpeedwiT/KouroshPanel/master/install.sh)${plain}"
    echo ""
...
reset_user() {
    ...
    if [[ $twoFactorConfirm != "y" && $twoFactorConfirm != "Y" ]]; then
        ${kourosh_folder}/kourosh setting -username "${config_account}" -password "${config_password}" > /dev/null 2>&1
    else
        ${kourosh_folder}/kourosh setting -username "${config_account}" -password "${config_password}" -resetTwoFactor=true > /dev/null 2>&1
        echo -e "Two factor authentication has been disabled."
    fi

    echo -e "Panel login username has been reset to: ${green} ${config_account} ${plain}"
    echo -e "Panel login password has been reset to: ${green} ${config_password} ${plain}"
    echo -e "${green} Please use the new login username and password to access the KOUROSH panel. Also remember them! ${plain}"
    confirm_restart
}
...
reset_webbasepath() {
    ...
    ${kourosh_folder}/kourosh setting -webBasePath "${config_webBasePath}" > /dev/null 2>&1
...
reset_config() {
    ...
    ${kourosh_folder}/kourosh setting -reset
    echo -e "All panel settings have been reset to default."
    restart
}

check_config() {
    local info=$(${kourosh_folder}/kourosh setting -show true)
    if [[ $? != 0 ]]; then
    ...
    if [[ -r "$db_env_file" ]] && grep -q '^XUI_DB_TYPE=postgres' "$db_env_file"; then
        ...
        echo -e "${green}Database: SQLite (/etc/kourosh/kourosh.db)${plain}"
    fi

    local existing_webBasePath=$(echo "$info" | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(echo "$info" | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_cert=$(${kourosh_folder}/kourosh setting -getCert true | grep 'cert:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
    local URL_lists=(
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        ${kourosh_folder}/kourosh setting -port ${port}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        check_status
        if [[ $? == 0 ]]; then
            LOGI "kourosh Started Successfully"
        else
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            LOGI "kourosh and xray stopped successfully"
        else
...
restart() {
    if [[ "${running_in_docker}" == "true" ]]; then
        ...
        check_status
        if [[ $? == 0 ]]; then
            LOGI "kourosh and xray Restarted successfully"
        else
    ...
    if [[ $release == "alpine" ]]; then
        rc-service kourosh restart
    else
        systemctl restart kourosh
    fi
    sleep 2
    check_status
    if [[ $? == 0 ]]; then
...
restart_xray() {
    ...
    if [[ $release == "alpine" ]]; then
        rc-service kourosh reload
    else
        systemctl reload kourosh
    fi
    LOGI "xray-core Restart signal sent successfully, Please check the log information to confirm whether xray restarted successfully"
    sleep 2
    show_xray_status
...
status() {
    ...
    if [[ $release == "alpine" ]]; then
        rc-service kourosh status
    else
        systemctl status kourosh -l
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
enable() {
    ...
    if [[ $release == "alpine" ]]; then
        rc-update add kourosh default
    else
        systemctl enable kourosh
    fi
    if [[ $? == 0 ]]; then
        LOGI "kourosh Set to boot automatically on startup successfully"
    else
        LOGE "kourosh Failed to set Autostart"
    fi
...
disable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "kourosh Autostart Cancelled successfully"
    else
        LOGE "kourosh Failed to cancel autostart"
    fi
    ...
    if [[ $release == "alpine" ]]; then
        ...
        case "$choice" in
            0)
                ...
                ;;
            1)
                grep -F 'kourosh[' /var/log/messages
                if [[ $# == 0 ]]; then
                    before_show_menu
                fi
                ;;
        ...
        case "$choice" in
            0)
                ...
                ;;
            1)
                journalctl -u kourosh -e --no-pager -f -p debug
                if [[ $# == 0 ]]; then
                    before_show_menu
                fi
                ;;
...
disable_bbr() {
    ...
    if [ -f "/etc/sysctl.d/99-bbr-kourosh.conf" ]; then
        old_settings=$(head -1 /etc/sysctl.d/99-bbr-kourosh.conf | tr -d '#')
        ...
        rm /etc/sysctl.d/99-bbr-kourosh.conf
    else
...
enable_bbr() {
    ...
    if [ -d "/etc/sysctl.d/" ]; then
        ...
        } > "/etc/sysctl.d/99-bbr-kourosh.conf"
        if [ -f "/etc/sysctl.conf" ]; then
        ...
        sysctl -p /etc/sysctl.d/99-bbr-kourosh.conf
    else
...
update_shell() {
    if replace_kourosh_script "https://github.com/SpeedwiT/KouroshPanel/raw/main/k-ui.sh" "true"; then
...
kourosh_pid() {
    ps -ef 2> /dev/null | grep -F "${kourosh_folder}/kourosh" | grep -v grep | awk 'NR==1 {print $1}'
}
...
check_status() {
    if [[ "${running_in_docker}" == "true" ]]; then
        if [[ ! -x "${kourosh_folder}/kourosh" ]]; then
            return 2
        fi
        if [[ -n "$(kourosh_pid)" ]]; then
            return 0
        else
            return 1
        fi
    ...
    if [[ $release == "alpine" ]]; then
        if [[ ! -f /etc/init.d/kourosh ]]; then
            return 2
        fi
        if [[ $(rc-service kourosh status | grep -F 'status: started' -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
    else
        if [[ ! -f ${kourosh_service}/kourosh.service ]]; then
            return 2
        fi
        temp=$(systemctl status kourosh | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    if [[ $release == "alpine" ]]; then
        if [[ $(rc-update show | grep -F 'kourosh' | grep default -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
    else
        temp=$(systemctl is-enabled kourosh)
...
show_mtproto_status() {
    local cfg_dir="${kourosh_folder}/bin/mtproto"
    local cfgs=()
    if [[ -d "${cfg_dir}" ]]; then
...
update_geofiles() {
    ...
    for dat in "${dat_files[@]}"; do
        ...
        remote_file="${dat%%_*}"
        local dest="${kourosh_folder}/bin/${dat}.dat"
...
ssl_cert_issue_main() {
    ...
    case "$choice" in
            ...
            if [ -z "$domains" ]; then
                ...
                if echo "$domains" | grep -qw "$domain"; then
                    ...
                    if [[ "${existing_cert}" == "/root/cert/${domain}/"* ]]; then
                        ${kourosh_folder}/kourosh cert -reset
        ...
        4)
            ...
            local panel_cert=$(${kourosh_folder}/kourosh setting -getCert true | grep 'cert:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
        ...
        5)
            ...
            if [[ "$pathChoice" == "2" ]]; then
                ...
                if [[ -f "${webCertFile}" && -f "${webKeyFile}" ]]; then
                    ${kourosh_folder}/kourosh cert -webCert "$webCertFile" -webCertKey "$webKeyFile"
                    ...
                    if [[ -f "${webCertFile}" && -f "${webKeyFile}" ]]; then
                        ...
                        if command -v ~/.acme.sh/acme.sh &> /dev/null && ~/.acme.sh/acme.sh --list 2> /dev/null | awk '{print $1}' | grep -Fxq "${domain}"; then
                            ~/.acme.sh/acme.sh --installcert --force -d "${domain}" \
                                ...
                                --reloadcmd "kourosh restart" 2>&1 || true
...
ssl_cert_issue_for_ip() {
    ...
    local existing_webBasePath=$(${kourosh_folder}/kourosh setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(${kourosh_folder}/kourosh setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    ...
    local reloadCmd="systemctl restart kourosh 2>/dev/null || rc-service kourosh restart 2>/dev/null"
...
ssl_cert_issue() {
    ...
    LOGI "Default --reloadcmd for ACME is: ${yellow}kourosh restart"
    LOGI "This command will run on every certificate issue and renew."
    read -rp "Would you like to modify --reloadcmd for ACME? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; kourosh restart"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
            1)
                LOGI "Reloadcmd is: systemctl reload nginx ; kourosh restart"
                reloadCmd="systemctl reload nginx ; kourosh restart"
                ;;
            2)
                LOGD "It's recommended to put kourosh restart at the end, so it won't raise an error if other services fails"
                read -rp "Please enter your reloadcmd (example: systemctl reload nginx ; kourosh restart): " reloadCmd
...
# setup_fail2ban_iplimit directly via `kourosh setup-fail2ban`.
install_iplimit() {
    setup_fail2ban_iplimit
    before_show_menu
}
...
EOF
    ...
    local panel_port
    panel_port=$(${kourosh_folder}/kourosh setting -show true 2>/dev/null | grep -Eo 'port: .+' | awk '{print $2}')
...
SSH_port_forwarding() {
    ...
    local existing_webBasePath=$(${kourosh_folder}/kourosh setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(${kourosh_folder}/kourosh setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_listenIP=$(${kourosh_folder}/kourosh setting -getListen true | grep -Eo 'listenIP: .+' | awk '{print $2}')
    local existing_cert=$(${kourosh_folder}/kourosh setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
    local existing_key=$(${kourosh_folder}/kourosh setting -getCert true | grep -Eo 'key: .+' | awk '{print $2}')
    ...
    case "$num" in
        1)
            if [[ -z "$existing_listenIP" || "$existing_listenIP" == "0.0.0.0" ]]; then
                ...
                ${kourosh_folder}/kourosh setting -listenIP "${config_listenIP}" > /dev/null 2>&1
            ...
            ;;
        2)
            ${kourosh_folder}/kourosh setting -listenIP 0.0.0.0 > /dev/null 2>&1
...
pg_ensure_hba_password_auth() {
    ...
    tmp=$(mktemp) || return 1
    {
        echo "# Added by kourosh: allow password logins for the panel database."
...
# Installs a local PostgreSQL server and creates a dedicated kourosh user/database.
...
# needs this:   kourosh pgclient <major>
pg_upgrade_client() {
    local want="$1" have
...
pg_install_server_action() {
    if postgresql_installed; then
        ...
        confirm "Run setup anyway (ensures the kourosh database/user exist)?" "n" || return 0
    fi
...
migrate_to_postgres() {
    ...
    if ! ${kourosh_folder}/kourosh migrate-db --dsn "$dsn"; then
    ...
    pg_write_env "$dsn"
    LOGI "Wrote database settings to $(kourosh_env_file_path) (XUI_DB_TYPE=postgres)."
...
postgresql_menu() {
    echo -e "${green}\t1.${plain} ${green}Install${plain} PostgreSQL (server + client + kourosh db)"
...
# file using the bundled kourosh binary. With no arguments it dumps the installed
# panel database; an optional second argument overrides the output path.
#   kourosh migrateDB [file.db|file.dump] [output]
migrate_db() {
    local input="$1" output="$2"
    local default_db="/etc/kourosh/kourosh.db"
    local bin="${kourosh_folder}/kourosh"

    [[ -z "$input" ]] && input="$default_db"

    if [[ ! -x "$bin" ]]; then
        LOGE "kourosh binary not found at ${bin}. Is the panel installed?"
        return 1
    fi

    if ! "$bin" migrate-db -h 2>&1 | grep -q -- '-dump'; then
        LOGE "This kourosh build does not support .db <-> .dump conversion yet."
        LOGE "Update the panel first (kourosh update) to a version with 'migrate-db --dump/--restore'."
        return 1
    fi

    if [[ ! -f "$input" ]]; then
        LOGE "Input file not found: ${input}"
        echo -e "Usage: ${green}kourosh migrateDB [file.db|file.dump] [output]${plain}"
        return 1
    fi
    ...
    if [[ "$mode" == "dump" ]]; then
        ...
        if [[ "$output" == "$default_db" ]] && check_status > /dev/null 2>&1; then
            LOGE "Refusing to restore into the live database (${default_db}) while kourosh is running."
            LOGE "Stop the panel first (kourosh stop) or choose a different output path."
            return 1
        fi
        if [[ -f "$output" ]]; then
            confirm "Output ${output} already exists and will be overwritten. Continue?" "n" || return 0
...
│                                                                │
│  ${blue}kourosh${plain}                       - Admin Management Script          │
│  ${blue}kourosh start${plain}                 - Start                            │
│  ${blue}kourosh stop${plain}                  - Stop                             │
│  ${blue}kourosh restart${plain}               - Restart                          │
|  ${blue}kourosh restart-xray${plain}          - Restart Xray                     │
│  ${blue}kourosh status${plain}                - Current Status                   │
│  ${blue}kourosh settings${plain}              - Current Settings                 │
│  ${blue}kourosh enable${plain}                - Enable Autostart on OS Startup   │
│  ${blue}kourosh disable${plain}               - Disable Autostart on OS Startup  │
│  ${blue}kourosh log${plain}                   - Check logs                       │
│  ${blue}kourosh banlog${plain}                - Check Fail2ban ban logs          │
│  ${blue}kourosh update${plain}                - Update                           │
│  ${blue}kourosh update-dev${plain}            - Update to Dev channel (latest)   │
│  ${blue}kourosh update-all-geofiles${plain}   - Update all geo files             │
│  ${blue}kourosh migrateDB [file]${plain}      - Convert .db <-> .dump (SQLite)   │
│  ${blue}kourosh pgclient [ver]${plain}        - Upgrade pg_dump/pg_restore tools │
│  ${blue}kourosh legacy${plain}                - Legacy version                   │
│  ${blue}kourosh install${plain}               - Install                          │
│  ${blue}kourosh uninstall${plain}             - Uninstall                        │
└────────────────────────────────────────────────────────────────┘"
}
